...
namespace ServiPuntos.API.Controllers
{
    ...
    public class PayPalController : ControllerBase
    {
            ...
            var mensaje = new MensajeNAFTA
        {
            ...
            var redirectUrl = $"http://servipuntosuy.up.railway.app:3000/paypal-return?status={Uri.EscapeDataString(status)}&paymentId={Uri.EscapeDataString(paymentId)}&payerId={Uri.EscapeDataString(PayerID)}&token={Uri.EscapeDataString(token)}";
        ...
        public IActionResult PayPalCancel([FromQuery] string token)
        {
            ...
            var redirectUrl = $"http://servipuntosuy.up.railway.app:3000/paypal-cancel?token={Uri.EscapeDataString(token)}";
            return Redirect(redirectUrl);
        }
    }
}